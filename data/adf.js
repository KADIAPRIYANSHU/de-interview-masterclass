window.adfLessons = {
    ...
    "tableData": {
        ...
        "rows": [
            [
                ...
                "Serverless / Managed by Microsoft",
                "Standard cloud-to-cloud data copies & Data Flows"
            ],
            [
                "Self-Hosted IR",
                "Private Network / On-Prem to Cloud",
                "Self-managed (Install VM, configure OS)",
                "Secure database ingestion (e.g., SAP, Oracle on-prem)"
            ],
            [
                "Azure-SSIS IR",
                "Any (via packages)",
                "Managed VMs (Configurable node size)",
                "Migrating SQL Server SSIS packages to Cloud"
            ]
        ]
    },
    "interviewQuestions": [
    ...
    "duration": "🕒 15 min read",
    "difficulty": "Advanced",
    "theory": "\n            <h3>Managed Virtual Network (Managed VNet)</h3>\n            <p>By default, Azure Data Factory copies data using public network endpoints. To secure sensitive data, enterprises deploy ADF inside a <strong>Managed VNet</strong>. In this mode, ADF provisions isolated compute nodes inside an Azure VNet managed by Microsoft, meaning raw data transit occurs off the public internet.</p>\n\n            <h3>Private Endpoints & Private Links</h3>\n            <p>A <strong>Private Endpoint</strong> is a private IP address assigned from your virtual network to a specific Azure service resource (like Azure SQL Database, ADLS Gen2, or Azure Key Vault). By setting up Private Endpoints within ADF's Managed VNet, your pipelines communicate with databases and files securely over the Microsoft backbone network, bypassing firewall public IPs entirely.</p>\n        ",
    "hasDiagram": false,
    "hasTable": true,
    "tableData": {
        ...
        "headers": [
            ...
            "Managed VNet + Private Endpoint"
        ],
        "rows": [
            [
                "Network Path",
                "Public internet IP / Azure Edge network",
                "Microsoft Azure Backbone (Private IP)"
            ],
            [
                "Firewall Config",
                "Must open service firewalls to 'Allow Azure Services'",
                "Completely block public IP; allow only Private Endpoint traffic"
            ],
            [
                "Egress Security",
                "Higher vulnerability to data exfiltration",
                "Highly secure; locked to target resources"
            ]
        ]
    },
    "interviewQuestions": [
    ...
    "duration": "🕒 15 min read",
    "difficulty": "Intermediate",
    "theory": "\n            <h3>Parameterized Datasets</h3>\n            <p>Instead of creating 100 separate Blob storage datasets for 100 different file schemas, you should create one single <strong>Parameterized Dataset</strong>. By defining parameters (e.g., <code>folderName</code> and <code>fileName</code>) inside the dataset settings, you pass values dynamically from the pipeline level during runtime.</p>\n            <h3>Dynamic Path Configurations</h3>\n            <p>You configure file paths in the connection settings using ADF Expression Language:</p>\n            <p><code>@dataset().folderName</code></p>\n            <p>This single dataset can then be used to read or write files to any S3/ADLS folder dynamically, reducing resource count in your project.</p>\n        ",
    "hasDiagram": false,
    "hasTable": false,
    "interviewQuestions": [
        {
    ...
    "tableData": {
        ...
        "rows": [
            [
                "Modifiability",
                "Read-only (static)",
                "Writable (Set/Append activities)"
            ],
            [
                "Set By",
                "Triggers or manual input at start",
                "Expressions inside pipeline run"
            ],
            [
                "Best For",
                "Environment settings, execution dates",
                "Counter tracking, dynamic values accumulator"
            ]
        ]
    },
    "interviewQuestions": [
    ...
    "tableData": {
        ...
        "rows": [
            [
                "Backfill Data",
                "No (only runs forward)",
                "Yes (can configure historical start date)",
                "No"
            ],
            [
                "Self-Dependency",
                "No (runs concurrently)",
                "Yes (stops subsequent windows on fail)",
                "No"
            ],
            [
                "Trigger Event",
    ...
    "tableData": {
        ...
        "rows": [
            ...
            [
                "Data Storage Link",
                "Must mount external stages",
                "Native link to OneLake storage"
            ],
            [
                "Pricing Model",
                "Pay-per-execution (activities, IRs)",
                "Capacity-based subscription (F-SKUs)"
            ]
        ]
    },
    "interviewQuestions": [
    ...
    "tableData": {
        ...
        "rows": [
            ...
            [
                "Integration Runtime",
                "Yes (configure Azure/SHIR)",
                "No (managed compute)"
            ],
            [
                "Billing unit",
                "Activity Runs + DIU-hours",
                "Fabric CUs (shared pool)"
            ],
            [
                "Lineage",
                "Requires Azure Purview",
                "Built-in to Fabric"
            ]
        ]
    },
    "interviewQuestions": [
        {
            "question": "What is the biggest operational difference between ADF and Fabric Data Factory?",
            "answer": "ADF is PaaS — you configure Linked Services, Integration Runtimes, and Key Vault integration manually. Fabric Data Factory is SaaS — compute is fully managed, and pipelines write directly to OneLake without any Linked Service configuration for Fabric-native stores. This dramatically reduces infrastructure overhead for Fabric-first architectures."
        },
        {
            "question": "Can Fabric Data Factory replace ADF for on-premises data ingestion?",
            "answer": "Not directly today. Fabric Data Factory does not support Self-Hosted Integration Runtime for on-premises sources behind firewalls. ADF with SHIR remains the recommended approach for hybrid on-premises to cloud ingestion scenarios until Fabric adds equivalent gateway support."
        }
    ]
},
    "4.5": {
    ...
    "interviewQuestions": [
        {
            "question": "When would you use an Azure Function Activity instead of a Web Activity in ADF?",
            "answer": "Use Azure Function Activity when the endpoint is specifically an Azure Function and you want native Managed Identity authentication, easier Linked Service management, and cleaner error reporting from Function-specific HTTP error codes. Use Web Activity for generic third-party HTTP REST endpoints where you manage auth headers manually."
        },
        {
            "question": "How do you pass pipeline parameters to an Azure Function Activity?",
            "answer": "Set the Body field of the Azure Function Activity to a JSON object using dynamic content expressions, for example: @{pipeline().parameters.runDate}. The Azure Function receives this JSON in the request body and can parse it to drive processing logic."
        }
    ]
},
    "4.6": {
    ...
    "interviewQuestions": [
        {
            "question": "What happens when a Lookup Activity returns more than 5,000 rows in ADF?",
            "answer": "ADF silently truncates the result to 5,000 rows. No error is raised. This is a common silent bug in control-table-driven pipelines where developers assume all entities are returned. Always validate with COUNT(*) before relying on Lookup for large control tables."
        },
        {
            "question": "How would you design a meta-driven ADF pipeline that processes 50,000 tables?",
            "answer": "Partition the control table into batches of 4,000 rows using a batch_id column. A root pipeline does a Lookup for distinct batch_ids (well within 5,000), then a ForEach with Batch Count 50 triggers child pipelines — each child does its own Lookup for its batch_id slice. This processes all 50,000 tables with up to 50 pipelines running in parallel."
        }
    ]
},
    "1.15": {
    ...
    "title": "Key ADF Activities",
    "subtitle": "Execute Pipeline, Until, Switch, Filter, Validation.",
    ...
    "module": "Parameters",
    "title": "Parameters vs Variables",
    "subtitle": "State management in ADF.",
    "duration": "🕒 5 min read",
    "difficulty": "Beginner",
    ...
    "id": "1.17",
    "stage": "Stage 1: Core Concepts & IRs",
    "module": "Triggers & Monitoring",
    "title": "Triggers & Runs",
    "subtitle": "Scheduling and Auditing.",
    "duration": "🕒 10 min read",
    "difficulty": "Intermediate",
